$("#time_warn").click(function () {
    var $prepare = $("#prepare_time");
    $prepare.toggle();

    //隐藏元素时,不置入表单
    if ($prepare.css("display") !== "none") {
        $prepare.find("input").attr("name", "prepare_time")
    } else {
        $prepare.find("input").attr("name", "")
...
function construct_img_path(status, img_id) {
    return '/static/images/' + status + '/' + img_id + '.gif';
}
...
function blink_img(img_id) {
    var i = 0;
    var $img = $("#img" + img_id);
    var before_src = $img.attr("src");
    var warn_src = construct_img_path("warning", img_id);
    $img.css("cursor", "pointer");
    $img.click(add_submit_page);
